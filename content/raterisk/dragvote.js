...
if (isNaN(pageIndex)) {
  votingOn = false;
  console.log(`no vote registered for this page`)
}

let resultTime = 60000;
...
const irlResultBox = document.getElementById("IRLResults");
const irlResults = Array.from(document.querySelectorAll("#IRLResults .voteChoice"));
const stats = Array.from(document.getElementsByClassName("irlStat"));
...
function trickleResults() {
  const resultTopper = document.getElementById("resultTopper");
  resultTopper.classList.remove("hidden");
  contextBox.style.display = "block";
  instructionBox.style.opacity = "0";
  console.log({positions});

const userAnswers = positions.map(item => {
  const classes = item.className.split(' ');
  return classes[1];
});

// Define the correct order array
const correctOrder = ['firstDrag', 'secondDrag', 'thirdDrag', 'fourthDrag', 'fifthDrag'];

function loopResults(index) {
      irlResults[index].style.opacity = "1";
      stats.forEach(stat => { stat.style.opacity = "0" });
      stats[index].style.opacity = "1";
      if (userAnswers[index] !== correctOrder[index]) {
        console.log(`answer index : ${userAnswers[index]} compared to ${correctOrder[index]}`)
          positions[index].classList.add('xed');
      } else {
          positions[index].classList.remove('xed');
      }
