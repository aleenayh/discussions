...
function updateChartLabels() {
  let labelData = labels.map((label) => stripHTMLTags(label.innerHTML));
...
let oldChart;
if (priorIndex) { oldChart = new Chart(priorResultsChart, {
...
firstChart = new Chart(resultsChart, {
...
function updateChartWithData(chart, data) {
  // Check if 'counts' property exists in data
  if (data && data.counts) {
    const countsData = data.counts;
    chart.data.datasets[0].data = countsData;
    chart.update();
  } else {
    console.error("Error: 'counts' property not found in the data object.");
  }
}
