...
const solos = Array.from(document.querySelectorAll(".soloTexts p"));
let solosIndex = 0;
solos.forEach(solo => {solo.style.opacity = "0"});
...
let contextReflection = document.getElementById("contextReflectionQs");
...
function advanceProgressSolos() {
  let adjustedArrayLength = solos.length;
  let adjustedIndex = solosIndex;
  let progress = (adjustedIndex / adjustedArrayLength) * 100;
  if (progressBar) {
    progressBar.style.width = progress + '%';
  }
...
if (images[currentIndex].tagName.toLowerCase() === 'video') {
images[currentIndex].pause();
images[currentIndex].currentTime = 0;
}
images[newIndex].classList.add("displayed");
if (images[newIndex].tagName.toLowerCase() === 'video') {
images[newIndex].play();
}
}

function fadeInReflection() {
  contextReflection.style.opacity = 1;
}

async function transitionSoloTexts() {
  if (solosIndex >= solos.length) {
    return;
  }
  let soloText = solos[solosIndex];
  textContainer.appendChild(soloText);
  await fadeInText(soloText);
  solosIndex++; 
  advanceProgressSolos();
  if (solosIndex >= solos.length) { //don't fade out last solo text
    return;
  } else { //fade out and launch the next iteration
  setTimeout(() => {fadeOutText(soloText)},8000);
  setTimeout(() => {textContainer.removeChild(soloText)},10000);
  setTimeout(transitionSoloTexts, 10000);
  }
}
...
async function transitionText() {
  ...
  if (stringsIndex >= strings.length) {
    if (contextReflection) {
      setTimeout(fadeInReflection,7000);
    }}
  fadeRunning = false;
}
